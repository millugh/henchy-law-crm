...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      throw new AppError('Failed to fetch letter templates', 'FETCH_ERROR', error)
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error) {
      throw new AppError('Failed to create letter template', 'CREATE_ERROR', error)
    }

    await logAuditEvent('CREATE', 'letter_templates', data.id, null, data)
